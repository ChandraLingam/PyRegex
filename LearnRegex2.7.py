...
#   Chandra Lingam
#
#   Compatible with 2.7 version of python
...
class LearnRegex(QtGui.QWidget):
    ...
    def match_click(self):
        ...
        try:
            #print (self.txtPattern.text(), type(self.txtPattern.text()))
            #self.regex = re.compile(self.txtPattern.text())
            self.regex = re.compile(unicode(self.txtPattern.text()))
        except Exception as e:           
            QtGui.QMessageBox.critical(self, 'Error', 'Pattern error: {0}'.format(str(e)))#.message))
            return
            
        self.regex_text = self.txtText.toPlainText()
        self.regex_group_index = dict((v,k) for k,v in self.regex.groupindex.iteritems())
        self.match_iter = self.regex.finditer(unicode(self.regex_text))
    ...
    def replace_click(self):
        self.txtResult.setText(re.sub(unicode(self.txtPattern.text()), unicode(self.txtRepPattern.text()), unicode(self.txtText.toPlainText())))
                        
    def split_click(self):
        self.txtResult.setText("")
        for s in re.split(unicode(self.txtPattern.text()), unicode(self.txtText.toPlainText())):
    ...
    def highlight_match(self):
        ...
        try:
            ...
            match = self.match_iter.next()
            end_time = time.time()
            
            self.txtResult.append (u'{0},{1},{2},{3:.2f}'.format(match.group(0),match.start(), match.end()-match.start(), end_time - start_time))
            ...
            for i in range(0, len(match.groups())):
                ...
                self.txtResult.append (u'  Group:{0}, Name:{1}, Value: {2}'.format(gn, group_name, match.group(gn)))
                
            self.highlighter(match.start(), match.end())
            
        except StopIteration:
            end_time = time.time()            
            self.txtResult.append (u'End, Elapsed Time (s): {0:0.2f}'.format(end_time - start_time))                            
